...
| 项目 | 内容 |
|---|---|
| 方案版本 | v0.3，2026-09-16 |
| 基线提交 | 26f47c494dd6b371312593e82f066713f6f56e9c |
| 当前状态 | M2/M3 已实现；M4 确定性对照与消融已运行，真实模型实验未运行 |
| 主线 | 退货申请的约束一致性、执行失败恢复、故障注入验证 |

本文件是经过保密整理的技术方案。个人计划和上游历史工作记录保留在本机，本文只描述技术范围与验收方法。本文件保留原始设计；已实现范围和实测结果以 [执行架构](../after-sales-architecture.md) 与 [验证记录](../verification.md) 为准。
...
- M1：复现缺签收证据仍判可退的原版行为。
- M2：统一规则、审批绑定、审批前置及各入口事务复核。
- M3：持久操作标识、原子回执、提交确认不明时核实、有限重试、审批与检查点恢复。
- M4：22 类固定场景 × 3 次重复的 B0/B1/B2 对照与三个消融，以及并发 1/3/5 各 100 请求的本地服务观察。
- 真实模型保留集、模型费用和生产容量仍未验证，不预填结论。GitHub CI 配置已提供，远端执行状态见验证记录。

本轮范围仍以 Python 的退货申请为主，不涉及真实支付或跨系统退款保证。[实施与验证记录](../verification.md)
